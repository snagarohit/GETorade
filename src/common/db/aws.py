...
class SimpleDatabase:
    ...
    def __getitem__(self, key):
        try:
            return next(_KVModel.query(self.app.app, _KVModel.key == self.app.table + "/" + key)).value
        except StopIteration:
            return None

    def __setitem__(self, key, value):
        _KVModel(self.app.app, key = self.app.table + "/" + key, value = value).save()

    def __delitem__(self, key):
        _KVModel(self.app.app, key = self.app.table + "/" + key).delete()
